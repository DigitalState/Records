...
Feature: Deny access to non-authenticated users to access endpoints
  ...
  Scenario: Read an access
    When I add "Accept" header equal to "application/json"
    And I send a "GET" request to "/accesses/1de63f79-b22e-498f-a3da-75c3448f0e74"
    Then the response status code should be 401
    And the header "Content-Type" should be equal to "application/json"
    And the response should be in JSON
  ...
  Scenario: Edit an access
    When I add "Accept" header equal to "application/json"
    And I add "Content-Type" header equal to "application/json"
    And I send a "PUT" request to "/accesses/1de63f79-b22e-498f-a3da-75c3448f0e74" with body:
    """
    {}
    """
    Then the response status code should be 401
  ...
  Scenario: Delete an access
    When I add "Accept" header equal to "application/json"
    And I send a "DELETE" request to "/accesses/1de63f79-b22e-498f-a3da-75c3448f0e74"
    Then the response status code should be 401
    And the header "Content-Type" should be equal to "application/json"
    And the response should be in JSON